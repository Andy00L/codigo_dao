...
pub fn handler(ctx: Context<CastReputationVote>, vote_type: u8, justification: String) -> Result<()> {
    ...
    // Basic permission: require minimum reputation
    require!(
    ...
    // Increase governance category score slightly
    ...
    voter_profile.category_scores[1] = voter_profile.category_scores[1].saturating_add(increment);

    // Touch realm metrics
    ctx.accounts.realm.active_proposals = ctx.accounts.realm.active_proposals.saturating_add(0);
